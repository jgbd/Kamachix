...
router.get('/', function(req, res, next) {
  ...
  if(req.query.c == 1){
    var sql='select anio,cant_docentes_tc,cant_docentes_hc,relacion_docentes from "Datawarehouse".relacion_docentes_kpi';
...
router.post('/', function(req, res, next) {
  ...
  var sql='select anio,cant_docentes_tc,cant_docentes_hc,relacion_docentes from "Datawarehouse".relacion_docentes_kpi where anio BETWEEN $1 and $2';
  //aquui se crea la conexion a DB
  pool.connect(function(err, client, done) {
    if(err) {
      return console.error('error fetching client from pool', err);
