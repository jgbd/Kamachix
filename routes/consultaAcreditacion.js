...
router.get('/', function(req, res, next) {
  if(req.query.c == 1)
    ...
    var sql = 'SELECT p.abreviatura,aac.inicioacreditacion,aac.periodo,(aac.inicioacreditacion+aac.periodo*365) as finacreditacion, aac.programa, p.departamento, aac.gravedad, aac.mail, aac.resolucion FROM programas p JOIN acreditacion_alta_calidad aac ON snies=programa WHERE aac.activo=true AND p.nivel=$1 AND p.estado=true ORDER BY finacreditacion DESC';
  }
  else if (req.query.c == 3){
    var beforedata=[req.query.flag];
    var sql = 'SELECT p.snies,p.abreviatura FROM programas p WHERE p.nivel=$1 AND p.estado=true EXCEPT SELECT aac.programa,p.abreviatura FROM programas p JOIN acreditacion_alta_calidad aac ON snies=programa WHERE activo=true ORDER BY abreviatura DESC';
  }
