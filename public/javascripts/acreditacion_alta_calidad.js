...
function Load_Start(){//carga tabla y gráficos a partir de datos almacenados anteriormente
  ...
  $.ajax({
    ...
    success : function(json) {
      $("#lblper").html("Indicador Acreditación Alta Calidad años: "+json.rows[0].Anho+" a "+now.getFullYear());
      ...
      $("#graph1").change(function () {
        ...
        if($(this).val() === '2'){
          ...
          lineGraph(json.rows,'divgraph1','Nivel de Acreditación \n',json.fields[0].name,json.fields[3].name);
        }
        else if($(this).val() === '4'){
          areaGraph(json.rows,'divgraph1','Nivel de Acreditación \n',json.fields[0].name,json.fields[3].name);
        }
        else if($(this).val() === '5'){
          barGraph(json.rows,'divgraph1','Nivel de Acreditación \n',json.fields[0].name,json.fields[3].name,0,0);
        }
        else if($(this).val() === '6'){
          barGraph(json.rows,'divgraph1','Nivel de Acreditación \n',json.fields[0].name,json.fields[3].name,40,30);
        }
      ...
      columnGraph(json.rows,'divgraph1','Nivel de Acreditación \n',json.fields[0].name,json.fields[3].name,0,0);
      columnTwoGraph(json.rows,'divgraph3','Programas Acreditados\n vs\n Total de Programas Pregrado',json.fields[0].name,json.fields[1].name,json.fields[2].name,0,0,"Programas Acreditados","Total Programas");
      gaugesGraph(json.rows[json.rowCount-1].razon,'divgraph2','r','y','g',10,20, 'Acreditación \n Alta Calidad', '%');
    }
  });
}
      ...
      for (var j = json.rowCount-1; j >=0; j--) {
        ...
        if(intervalo<0)
          ...
          if (intervalo>0 && intervalo<=365){
            ...
            $("#tableresprogram").append('<td><img id="est" src="/images/verde.png" alt="GREEN" title="Acreditado"></td>');
          //----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        }
        $("#tableresprogram").append('</tr>');
       }
  ...
  if(ban){
    $.ajax({
      ...
      success : function(json) {
        ...
        columnTwoGraph(json.datos,'divgraph3','Programas Acreditados\n vs\n Total de Programas Pregrado',json.fieldstwo[0],json.fieldstwo[1],json.fieldstwo[2],0,0,"Programas Acreditados","Total Programas");
        gaugesGraph(json.datos[json.count-1].razon,'divgraph2','r','y','g',10,20, 'Acreditación \n Alta Calidad', '%');
      }
    });
  }
  closedivfilter();
