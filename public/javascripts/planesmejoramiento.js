  ...
  if($("#lstacre").val()!=0){
    ...
    $.ajax({
     ...
     success : function(json) {
       ...
       $("#imgestmen").html(calculate_state(fecfinacre, inf[0], 5, json[0].mailmen, json[0].chkmen));
       if (json[0].chkmen) {
       ...
       $("#imgestpm1").html(calculate_state(fecpm1, inf[0], 1, json[0].mailpm1, json[0].chkpm1));
       if (json[0].chkpm1) {
      ...
      //se da formato a la fecha fin auto evaluación 1 acreditación
       ...
       $("#imgestaev1").html(calculate_state(fecaev1, inf[0], 2, json[0].mailaev1, json[0].chkaev1));
       if (json[0].chkaev1) {
      ...
      //se da formato a la fecha fin plan mejoramiento 2  y auto evaluación 2 acreditación
       if (json[0].fpm2!='no aplica'){
        ...
        $("#imgestpm2").html(calculate_state(fecpm2, inf[0], 3, json[0].mailpm2, json[0].chkpm2));
        if (json[0].chkpm2) {
        ...
        $("#imgestaev2").html(calculate_state(fecave2, inf[0], 4, json[0].mailaev2, json[0].chkaev2));
...
function calculate_state(fecha, snies, plan, banmail, banest){
  var now = new Date();
  var resto = fecha.getTime()-now.getTime();
  resto = Math.floor(resto/(1000*60*60*24));
  var cad='';
  if(resto>0){
    if (resto<=30 && !banmail && !banest) {
      sendMail(snies, plan);
    }
