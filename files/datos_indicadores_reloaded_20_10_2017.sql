...
--
-- Definition for function fun_actua_formacion (OID = 19148) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function fun_actua_relacion (OID = 19149) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function fun_delete_cohort (OID = 19150) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function fun_delete_period (OID = 19151) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function Actua_Formacion_Dep (OID = 19152) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function Fun_Actua_KPI_Form (OID = 19153) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function fun_actua_satisfaccion (OID = 19154) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function Fun_Actua_KPI_Est_Doc (OID = 19382) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function Actua_Estudiantes_Dep (OID = 19384) : 
--
...
LANGUAGE plpgsql;
--
-- Structure for table KPI_Acreditacion (OID = 19155) : 
--
SET search_path = "Datawarehouse", pg_catalog;
...
WITH (oids = false);
--
-- Structure for table KPI_Desercion_Cohorte (OID = 19159) : 
--
...
WITH (oids = false);
--
-- Structure for table KPI_Desercion_Periodo (OID = 19163) : 
--
...
WITH (oids = false);
--
-- Structure for table KPI_Formacion (OID = 19172) : 
--
...
WITH (oids = false);
--
-- Structure for table KPI_Nivel_Satisfaccion (OID = 19176) : 
--
...
WITH (oids = false);
--
-- Structure for table KPI_Relacion_Docentes (OID = 19180) : 
--
...
WITH (oids = false);
--
-- Structure for table acreditacion_alta_calidad (OID = 19184) : 
--
SET search_path = public, pg_catalog;
...
WITH (oids = false);
--
-- Structure for table formacion (OID = 19205) : 
--
...
WITH (oids = false);
--
-- Structure for table formacion_departamento (OID = 19208) : 
--
...
WITH (oids = false);
--
-- Structure for table manuales_indicadores (OID = 19213) : 
--
...
WITH (oids = false);
--
-- Structure for table programas (OID = 19223) : 
--
...
WITH (oids = false);
--
-- Structure for table users (OID = 19228) : 
--
...
WITH (oids = false);
--
-- Structure for table KPI_Estudiantes_por_Docentes_TC (OID = 19351) : 
--
SET search_path = "Datawarehouse", pg_catalog;
...
WITH (oids = false);
--
-- Structure for table estudiantes_departamento (OID = 19373) : 
--
SET search_path = public, pg_catalog;
...
\.
--
-- Definition for index KPI_Desercion_Cohorte_idx (OID = 19241) : 
--
SET search_path = "Datawarehouse", pg_catalog;
...
--
-- Definition for index KPI_Formacion_fk1 (OID = 19305) : 
--
ALTER TABLE ONLY "KPI_Formacion"
    ADD CONSTRAINT "KPI_Formacion_fk1"
    FOREIGN KEY ("manual_Formacion") REFERENCES public.manuales_indicadores(codigo);
--
-- Definition for index KPI_Nivel_Satisfaccion_fk (OID = 19310) : 
--
ALTER TABLE ONLY "KPI_Nivel_Satisfaccion"
    ADD CONSTRAINT "KPI_Nivel_Satisfaccion_fk"
    FOREIGN KEY (manual) REFERENCES public.manuales_indicadores(codigo);
--
-- Definition for index KPI_Period_Dropout_fk (OID = 19315) : 
--
ALTER TABLE ONLY "KPI_Desercion_Periodo"
    ADD CONSTRAINT "KPI_Period_Dropout_fk"
    FOREIGN KEY (programa) REFERENCES public.programas(snies);
--
-- Definition for index KPI_Relacion_Docentes_fk (OID = 19320) : 
--
...
--
-- Definition for index KPI_Satisfaction_level_fk (OID = 19325) : 
--
ALTER TABLE ONLY "KPI_Nivel_Satisfaccion"
    ADD CONSTRAINT "KPI_Satisfaction_level_fk"
    FOREIGN KEY ("Programa") REFERENCES public.programas(snies);
--
-- Definition for index acreditacion_alta_calidad_fk (OID = 19330) : 
--
SET search_path = public, pg_catalog;
ALTER TABLE ONLY acreditacion_alta_calidad
...
--
-- Definition for index KPI_Estudiantes_por_Docentes_TC_fk1 (OID = 19366) : 
--
ALTER TABLE ONLY "KPI_Estudiantes_por_Docentes_TC"
    ADD CONSTRAINT "KPI_Estudiantes_por_Docentes_TC_fk1"
    FOREIGN KEY (departamento) REFERENCES public.users(codigo);
--
-- Definition for index estudiantes_departamento_fk (OID = 19377) : 
--
SET search_path = public, pg_catalog;
ALTER TABLE ONLY estudiantes_departamento
    ...
    FOREIGN KEY (departamento) REFERENCES users(codigo);
--
-- Definition for trigger tr_actua_satisfaccion (OID = 19264) : 
--
SET search_path = "Datawarehouse", pg_catalog;
...
--
-- Definition for trigger tr_cohort_desertion (OID = 19265) : 
--
...
--
-- Definition for trigger tr_period_desertion (OID = 19266) : 
--
...
--
-- Definition for trigger tr_relacion (OID = 19267) : 
--
...
--
-- Definition for trigger tr_for_dep (OID = 19268) : 
--
SET search_path = public, pg_catalog;
...
--
-- Definition for trigger tr_kpi_form (OID = 19269) : 
--
...
--
-- Definition for trigger tr_kpi_est_doc (OID = 19383) : 
--
...
--
-- Definition for trigger tr_est_dep (OID = 19385) : 
--
...
--
-- Data for sequence public.formacion_departamento_cod_forma_dep_seq (OID = 19211)
--
SELECT pg_catalog.setval('formacion_departamento_cod_forma_dep_seq', 11445, true);
--
-- Data for sequence public.estudiantes_departamento_cod_est_dep_seq (OID = 19371)
--
